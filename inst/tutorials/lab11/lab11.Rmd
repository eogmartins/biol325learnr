...
quiz(
  ...
  question(
    ...
    answer("A predictive model uses correlation; a causal model uses regression."),
...
lake |>
  ...
  ggplot(aes(x = value, y = algal_biomass)) +
    ...
    labs(x = "Standardized predictor value", y = "Algal Biomass")
```
...
The code below uses `est()` and `tribble()` to build a comparison table of your model estimates against the true DGVs that were used to simulate the data. Each row is a source of values (the true data-generating values [DGVs] or a model estimate), and each column is the causal effect of the variable name that appears to the left of the arrow in the column name. The `est()` calls sit directly in the cells where those model estimates belong, keeping the structure clear at a glance.

**Your Turn:** Just examine the code, then press **Submit Answer**.
...
**Nutrient Concentration** (true DGV: +0.50). The DAG-guided model recovers ≈ +0.48. Both AIC models give only ≈ +0.18, which is an attenuation of roughly 64%. The reason is that nutrient concentration has both a strong positive effect on algal biomass (+0.50) and a direct negative effect on dissolved oxygen (−0.20). When dissolved oxygen is in the model, it captures much of the variation in algal biomass that nutrient concentration would otherwise explain, dramatically suppressing its estimated coefficient.
...
**The bottom line:** Excluding a downstream variable (e.g., dissolved oxygen) from the candidate set is a meaningful improvement, as it eliminates the severe attenuation of water temperature, zooplankton biomass, and nutrient concentration effects. But it is not a substitute for causal reasoning. Getting the right answer for water temperature still requires knowing, from the DAG, that its total effect must be estimated from a model fitted without its mediators.")
})
```
...
**Water Temperature**: The AIC-with-DO model dropped water temperature entirely (no estimate shown). The AIC-without-DO CI is centred ≈ +0.44 and shifted left of the DGV (+0.50) because its mediators are in the same model. Only the DAG-guided CI is centred near the DGV dashed line, as it estimates the total effect from a model without mediators.")
